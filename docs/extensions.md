...
- `tool_authorization` (final pre-exec authorization after all `tool_call` rewrites and immediately before native approval; receives a detached view of the final execution `input`, native allow/ask decision, approval mode, and whether tool policy, explicit user policy, or provider safety requires a human; provider-native computer calls expose `actions` and `pendingSafetyChecks`; returns `{ decision: "allow" | "ask" | "deny", reason?: string }`)
